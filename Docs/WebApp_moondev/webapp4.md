...
The complete code was saved to a downloadable file! Here's how to access it:

## Download the File

Click on the file attachment in my previous response:
📎 **lumibot_strategy_generator_complete.py**

You can download it directly from there.

## Or Copy It Here

If you can't see the download link, here's the complete code you can copy and paste:
...
class StrategyAnalyzerSubagent:
    """Analyzes trading content and extracts strategy rules using Claude"""
...
def main():
    ...
    if not ANTHROPIC_API_KEY:
        st.error("⚠️ ANTHROPIC_API_KEY not found in environment variables. Please set it before running.")
    ...
    with col2:
        ...
        if st.button("🚀 Generate Lumibot Strategy", type="primary", use_container_width=True, disabled='content' not in st.session_state):
            ...
            analyzer = StrategyAnalyzerSubagent(ANTHROPIC_API_KEY)
            ...
            generator = LumibotCodeGeneratorSubagent(ANTHROPIC_API_KEY)
...
1. **Save this code** to a file named `lumibot_strategy_generator.py`
...
```
streamlit run lumibot_strategy_generator.py
```


The webapp will open in your browser automatically! This is the complete, working code ready to use.

<div align="center">⁂</div>

[^1]: https://github.com/moondevonyt/moon-dev-ai-agents
